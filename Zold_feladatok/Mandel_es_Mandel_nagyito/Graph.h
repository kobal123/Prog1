...
public:
	...
	void hide_all(){for(int i=0;i<v.size();i++){}}
...
struct Rectangle : Shape {
	...
	Rectangle(Point xy, int ww, int hh): w{ww}, h{hh}
	{
		...
		{
			cout<<h<<" "<<w<<endl;
			error("Nem jó oldalhossz. Rectangle");
		}
		add(xy);
	}
	Rectangle(Point x, Point y): w{ y.x - x.x }, h{ y.y - x.y }
	{
		if (h<=0 || w<=0) error("Nem jó pontok. Rectangle");
		add(x);
	}
...
{
public:
	Pixel(Point xy,uchar rr,uchar gg,uchar bb):x{xy.x},y{xy.y},r{rr},g{gg},b{bb}
		{
			if(x<0 || y<0) error("Pixel konstruktor, pixel kisebb mint 0");
		}
